...
CR_REPO_URL = "https://agentbase.api.vngcloud.vn/cr/api/v1/repository"
...
def get_repo_info(token):
    ...
    req = urllib.request.Request(CR_REPO_URL, headers=headers)
    with urllib.request.urlopen(req) as response:
...
if __name__ == "__main__":
    try:
        token = get_token()
        repo = get_repo_info(token)
        print(f"REGISTRY_URL={repo.get('registryUrl')}")
        print(f"REPO_NAME={repo.get('name')}")
